...
struct Engram {
    ...
    // Evolution
    supersedes: Option<Uuid>,      // This engram replaces an earlier one
    superseded_by: Option<Uuid>,   // This engram has been replaced by a newer one
}
...
- **Entity field**: Free-form key generated by Gemma during extraction. Used for
  supersession detection — when a new engram shares an entity with an existing one
...
Produces candidate engrams as JSON arrays.
...
8. Failures dumped to `~/.local/share/engram/debug/` for diagnosis
...
5. **Supersession**: Same entity + cosine 0.7-0.85 → new replaces old.
   ...
   engrams are skipped. Complementary facts (similarity < 0.7) coexist.
6. **Persist** to LanceDB with embedding

**Superseded engrams** are filtered from MCP recall results and CLI list output.
...
|--------|---------|---------------|
| `agent-aside_question-*` | `/btw` command — user asks a side question | **Yes** — high signal for engram |
...
via `/remember` and `CLAUDE.md`. Engram's value is the *implicit* knowledge
that accumulates without conscious effort. Write tools are roadmapped but
deferred until v1 proves the concept.

**Superseded filtering**: Results exclude engrams with `superseded_by` set.
...
Engram's reconciliation pipeline uses systemd's native inotify support
...
reconciliation) that scans project-scoped engrams across all projects, identifies
...
project-scoped engrams with reasoning attached are the raw material.

### Accessed-at Tracking
MCP recalls should update `accessed_at` on returned engrams. Enables future
decay/archival logic. Currently a TODO in `mcp/server.rs`.

### Retention and Pruning
Database grows unbounded. Needs:
- Archival of engrams not accessed in N months
- Possible TTL differences by category (humor/interest may decay faster)
- `engram archive` command
...
### Session-Start Hook
Use Claude Code's hook system to auto-run `engram_recall` at session start,
injecting results into context. Eliminates dependence on the model choosing
to call the tool.

### TUI Browser
`engram browse` — interactive terminal UI for exploring, filtering, and
managing engrams. Build with ratatui if CLI management feels limiting.
